...
void fitxategiBatSortu(MP* points, int pkop, DIM mapDim)
{
	...
	if (pkop <= 0 || distantzia == NULL) printf("101 Errorea");
		...
		dimentsioakIdatzi(fitxategia, mapDim); // Maparen dimentsioak gorde
		puntuakIdatzi(fitxategia, points, pkop, distantzia); // Puntuen id-a eta koordenatuak gorde
		distantziakIdatzi(fitxategia, pkop, distantzia); // Puntuen arteko distantzien matrizea gorde
		fclose(fitxategia);
	}
}
